...
public class DijkstraDriver {
    ...
    public static Map<City, Double> dijkstra(List<City> cities, City start) {
        ...
        for (City city : cities) {
            distances.put(city, Double.valueOf(Double.POSITIVE_INFINITY)); //stores initial distance value of each city to infinity
        }
        distances.put(start, Double.valueOf(0.0));
        pq.add(new CityDistance(start, Double.valueOf(0.0)));
        while (!pq.isEmpty()) {
            ...
            for (City next : cities) {
                ...
                if (totDistance < distances.get(next)){ // if total distance of current city being targeted is less than previous target add as new shortest path
                    distances.put(next, Double.valueOf(totDistance));
    ...
    public static List<City> result(List<City> cities, City start, City end){
        ...
        for (City city : cities) {
            distances.put(city, Double.valueOf(Double.POSITIVE_INFINITY)); //initial distance of unknown cities to infinity
        }
        distances.put(start, Double.valueOf(0.0));
        pq.add(new CityDistance(start, Double.valueOf(0.0)));
